...
async function seedDatabase() {
  ...
  const routeCauses = [
    "Fil détendu suite à usure du ressort tendeur", "Dérive du programme après mise à jour firmware",
    "Aiguille usée non détectée à temps", "Maintenance préventive planifiée",
    "Mauvais paramétrage opérateur en début de poste", "Cause non déterminée, surveillance renforcée",
  ];
  const actionPilotes = ["T. Benali (Process)", "Équipe Maintenance", "S. Haddad (Qualité)", "Responsable Ligne", "Fournisseur Amatec"];
  for (let i = 0; i < 10; i++) {
    ...
    await insertIntervention({
      ...
      routeCause: routeCauses[i % routeCauses.length],
      actionPilote: actionPilotes[i % actionPilotes.length],
      durationMinutes: 10 + Math.floor(Math.random() * 80),
...
/*
  ⚠️ MIGRATION SQL REQUISE (une seule fois, dans Supabase → SQL Editor) :

  ALTER TABLE interventions
    ADD COLUMN IF NOT EXISTS route_cause text,
    ADD COLUMN IF NOT EXISTS action_pilote text,
    ADD COLUMN IF NOT EXISTS duration_minutes numeric;

  Ces 3 colonnes remplacent l'usage de "comment" pour la saisie process :
  route_cause = analyse de la cause racine (obligatoire), action_pilote =
  qui pilote l'action corrective, duration_minutes = durée de l'intervention
  en minutes (utilisée pour l'analyse d'efficacité).
  L'ancienne colonne "comment" est conservée pour compat historique.
*/
async function insertIntervention(i) {
  ...
  const row = {
    ...
    comment: i.comment || "", route_cause: i.routeCause || "", action_pilote: i.actionPilote || "",
    duration_minutes: i.durationMinutes === undefined ? null : i.durationMinutes,
    projet: i.projet, ligne: i.ligne,
  };
...
async function checkCredentials(role, username, password) {
  const { data, error } = await sb.from("users").select("*").eq("username", username).eq("password", password).eq("active", true).limit(1);
