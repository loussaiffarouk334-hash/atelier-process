...
function renderPortal() {
  ...
  <div class="min-h-screen bg-slate-50 flex flex-col justify-between p-6 relative overflow-hidden select-none">
    ...
    <div class="flex flex-col items-center text-center mt-10 relative z-10">
      <div class="flex gap-1.5 mb-6 items-center">
        <button data-action="set-lang" data-lang="fr" class="font-mono font-bold text-[10px] py-1.5 px-3 rounded-md border text-center cursor-pointer tracking-wider transition-colors ${lang === "fr" ? "bg-indigo-600 border-indigo-600 text-white" : "bg-white border-slate-200 text-slate-400 hover:text-slate-700"}">FRANÇAIS</button>
        <button data-action="set-lang" data-lang="en" class="font-mono font-bold text-[10px] py-1.5 px-3 rounded-md border text-center cursor-pointer tracking-wider transition-colors ${lang === "en" ? "bg-indigo-600 border-indigo-600 text-white" : "bg-white border-slate-200 text-slate-400 hover:text-slate-700"}">ENGLISH</button>
        ${themeToggleButton("ml-1")}
      </div>
      ...
      <p class="text-slate-400 font-sans text-xs uppercase tracking-widest font-bold mt-1.5">${t("brand.sub")}</p>
    </div>
    ...
    <div class="max-w-4xl mx-auto w-full my-auto py-8 grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10 px-4">
      <button data-action="open-portal-password" data-role="process" class="group text-left bg-white border border-slate-200 hover:border-indigo-500/80 rounded-2xl p-8 shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5 flex flex-col justify-between h-[280px] cursor-pointer">
        <div>
          ...
          <h2 class="text-xl font-display font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">${t("portal.process.title")}</h2>
          <p class="text-slate-500 font-sans text-sm mt-3 leading-relaxed">${t("portal.process.desc")}</p>
        </div>
        <div class="flex items-center text-xs font-bold text-indigo-600/80 group-hover:text-indigo-600 mt-4">
          <span class="mr-1">${t("portal.process.cta")}</span> &rarr;
        </div>
      </button>
      ...
      <button data-action="open-portal-password" data-role="qualite" class="group text-left bg-white border border-slate-200 hover:border-emerald-500/80 rounded-2xl p-8 shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5 flex flex-col justify-between h-[280px] cursor-pointer">
        <div>
          ...
          <h2 class="text-xl font-display font-bold text-slate-800 group-hover:text-emerald-600 transition-colors">${t("portal.qualite.title")}</h2>
          <p class="text-slate-500 font-sans text-sm mt-3 leading-relaxed">${t("portal.qualite.desc")}</p>
        </div>
        <div class="flex items-center text-xs font-bold text-emerald-600/80 group-hover:text-emerald-600 mt-4">
          <span class="mr-1">${t("portal.qualite.cta")}</span> &rarr;
        </div>
      </button>
    </div>
...
function renderPasswordModal(role, errorMsg) {
  const cfg = role === "process"
    ? { title: t("pw.title.process"), color: "indigo" }
    : { title: t("pw.title.qualite"), color: "emerald" };
  return `
  <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" data-action="close-password-modal-backdrop">
    <div class="bg-white border border-slate-200 rounded-2xl max-w-sm w-full shadow-xl overflow-hidden anim-modal">
      <div class="p-6">
        ...
        <form data-action="submit-password" data-role="${role}" class="space-y-4">
          <div>
            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">${t("pw.username")}</label>
            <input type="text" name="username" placeholder="identifiant" autofocus autocomplete="username"
              class="w-full bg-slate-50 border border-slate-200 rounded-lg py-2 px-3 text-slate-800 text-sm focus:outline-none focus:border-${cfg.color}-600" />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">${t("pw.label")}</label>
          ...
          <div class="flex gap-2 justify-end pt-2">
            <button type="button" data-action="close-password-modal" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-semibold text-xs py-2.5 px-4 rounded-lg cursor-pointer">${t("pw.cancel")}</button>
            <button type="submit" class="bg-${cfg.color}-600 hover:bg-${cfg.color}-700 text-white font-semibold text-xs py-2.5 px-4 rounded-lg cursor-pointer shadow-sm">${t("pw.submit")}</button>
          </div>
        </form>
      </div>
    </div>
