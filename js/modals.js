...
function modalShell(title, colorClass, formHtml) {
  ...
  <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" data-action="close-modal-backdrop">
    <div class="bg-white border border-slate-200 rounded-2xl max-w-2xl w-full shadow-xl overflow-hidden anim-modal max-h-[90vh] flex flex-col" onclick="event.stopPropagation()">
      <div class="p-6 pb-4 flex items-center justify-between border-b border-slate-100">
        ...
        <button data-action="close-modal" class="text-slate-400 hover:text-slate-600 text-xl cursor-pointer">&times;</button>
      </div>
...
function modalIntervention(state, data) {
  ...
  const v = item || {
    ...
    scrap: 0, scrap_qty: 0, comment: "", projet: PROJETS[0], ligne: "",
  };
    ...
    <form data-action="submit-form" data-form="intervention" data-id="${id || ""}" class="space-y-4">
      <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        ${field("Machine", `<select name="machine_id" id="intervention-machine" class="${inputCls} font-mono">
            ${machines.map((m) => `<option value="${esc(m.machine_id)}" ${v.machine_id === m.machine_id ? "selected" : ""}>${esc(m.machine_id)}</option>`).join("")}
          </select>`)}
        ${field("Part Number", `<input list="pn-list" name="part_number" value="${esc(v.part_number)}" class="${inputCls} font-mono" /><datalist id="pn-list">${pns.map((pn) => `<option value="${esc(pn)}">`).join("")}</datalist>`)}
      ...
      ${field("Commentaire", `<textarea name="comment" rows="2" class="${inputCls}">${esc(v.comment)}</textarea>`)}
  ...
  const formHtml = `
    <form data-action="submit-form" data-form="scrap" data-id="${id || ""}" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        ${field("Part Number", `<input list="pn-list-scrap" name="part_number" required value="${esc(v.part_number)}" class="${inputCls} font-mono" /><datalist id="pn-list-scrap">${pns.map((pn) => `<option value="${esc(pn)}">`).join("")}</datalist>`, true)}
