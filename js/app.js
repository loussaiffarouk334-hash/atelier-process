...
const state = {
  ...
  currentUser: null, // { username, role } — rôle réel de l'utilisateur connecté (process | qualite | admin)
  drill: { level: "month", monthKey: null, dayKey: null, hourKey: null, minuteKey: null },
...
/* Redessine uniquement les <canvas> (sans refaire un appel réseau ni reconstruire le DOM).
   Corrige les décalages/textes tronqués liés à un canvas mesuré avant que la police
   personnalisée soit chargée, ou après une rotation/redimensionnement de l'écran. */
function repaintCharts() {
  if (!lastData) return;
  if (state.screen === "process" && state.activePage === "dashboard") {
    paintDashboardChart(state, lastData);
  }
  if (state.screen === "quality") {
    paintQualityCharts(lastData);
  }
}
...
window.addEventListener("resize", debounce(repaintCharts, 150));
if (document.fonts && document.fonts.ready) {
  document.fonts.ready.then(() => repaintCharts());
}
...
function wireFilterInputs(data) {
  ...
  // Type (Programme / Machine) -> afficher le champ Section uniquement pour "Programme"
  const interventionTypeSelect = document.getElementById("intervention-type");
  if (interventionTypeSelect) {
    const toggleSectionField = () => {
      const wrapper = document.getElementById("intervention-section-wrapper");
      if (wrapper) wrapper.style.display = interventionTypeSelect.value === "Programme" ? "" : "none";
    };
    interventionTypeSelect.addEventListener("change", toggleSectionField);
    toggleSectionField();
  }
}
...
document.addEventListener("click", async (e) => {
  ...
  switch (action) {
    case "toggle-theme": {
      ...
      localStorage.setItem("atelier-theme", isLight ? "dark" : "light");
      location.reload();
      break;
    }
    case "set-lang":
    ...
    case "switch-screen":
      // Réservé à l'administrateur : bascule Process <-> Qualité sans ressaisir de mot de passe
    ...
    case "drill-back":
      if (state.drill.level === "hour" && state.drill.hourKey && state.drill.minuteKey) {
        state.drill = { level: "hour", monthKey: state.drill.monthKey, dayKey: state.drill.dayKey, hourKey: state.drill.hourKey, minuteKey: null };
      } else if (state.drill.level === "hour" && state.drill.hourKey) {
        state.drill = { level: "hour", monthKey: state.drill.monthKey, dayKey: state.drill.dayKey, hourKey: null, minuteKey: null };
      } else if (state.drill.level === "hour") {
        state.drill = { level: "day", monthKey: state.drill.monthKey, dayKey: null, hourKey: null, minuteKey: null };
      } else if (state.drill.level === "day") {
        state.drill = { level: "month", monthKey: null, dayKey: null, hourKey: null, minuteKey: null };
      } else {
        state.drill = { level: "month", monthKey: null, dayKey: null, hourKey: null, minuteKey: null };
      }
    ...
    // Fil d'Ariane texte de la courbe scrap : mois → jour → heure → minute.
    // Chaque segment cliqué ramène directement à ce niveau de zoom précis.
    case "drill-to": {
      const lvl = btn.dataset.level;
      if (lvl === "month") {
        state.drill = { level: "month", monthKey: null, dayKey: null, hourKey: null, minuteKey: null };
      } else if (lvl === "day") {
        state.drill = { level: "day", monthKey: btn.dataset.monthKey || null, dayKey: null, hourKey: null, minuteKey: null };
      } else if (lvl === "hour") {
        state.drill = { level: "hour", monthKey: btn.dataset.monthKey || null, dayKey: btn.dataset.dayKey || null, hourKey: null, minuteKey: null };
      } else if (lvl === "minute") {
        state.drill = { level: "hour", monthKey: btn.dataset.monthKey || null, dayKey: btn.dataset.dayKey || null, hourKey: btn.dataset.hourKey || null, minuteKey: null };
      }
      await renderApp();
      break;
    }
...
document.addEventListener("submit", async (e) => {
  ...
  if (form.dataset.action === "submit-password") {
    ...
    const user = await checkCredentials(role, username, password);
    if (user) {
      state.passwordPrompt = { open: false, role: null, error: null };
      ...
      state.passwordPrompt.error = "Identifiant ou mot de passe incorrect.";
      await renderApp();
    }
    return;
  }
...
async function handleSubmitForm(form) {
  ...
  if (type === "intervention") {
    ...
    await insertIntervention({
      ...
      type: get("type"), machineId: get("machine_id"), partNumber: get("part_number"),
      section: get("type") === "Programme" ? get("section") : "",
      ...
      routeCause: get("route_cause"), actionPilote: get("action_pilote"),
      durationMinutes: get("duration_minutes") === "" ? null : Number(get("duration_minutes")),
      projet: get("projet"), ligne: get("ligne"),
    });
