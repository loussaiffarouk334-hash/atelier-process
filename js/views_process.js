...
function renderDashboard(state, data) {
  ...
  else if (drill.level === "hour" && drill.hourKey) scrapDetailSub = t("table.scrapDetail.minute");
  ...
  if (canvas) {
    ...
    attachLineChartClick(canvas, geometry, (idx) => {
      if (drill.level === "month") {
        ...
        state.drill = { level: "hour", monthKey: drill.monthKey, dayKey: drill.dayKey, hourKey: keys[idx] };
        renderApp();
      }
      // Niveau minute (feuille) : le détail est déjà affiché dans le tableau sous la courbe.
    });
  }
}
