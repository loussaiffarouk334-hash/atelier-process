...
function renderProcessShell(state, data) {
  ...
  <div class="min-h-screen bg-slate-50 flex flex-col font-sans select-none relative">
    <div class="bg-slate-900 text-slate-300 text-center py-2 px-4 font-semibold text-xs tracking-wide shadow-inner border-b border-slate-950 relative z-30">
      🔒 ${t("nav.section.suivi") === "Suivi" ? "Espace Process" : "Process Workspace"} — SQLite (sql.js)
    </div>
  ...
  <div class="space-y-6" id="dashboard-root">
    ...
    <div class="bg-slate-950 rounded-2xl p-5 shadow-lg border border-slate-900 relative overflow-hidden">
      <div class="flex items-center justify-between mb-1 relative z-10">
        ...
        ${drill.level === "day"
          ? `<button data-action="drill-back" class="text-[11px] font-mono text-red-300 bg-red-950/60 border border-red-900 rounded-md px-3 py-1 hover:text-white">${t("chart.backMonths")}</button>`
          : `<button class="text-[11px] font-mono text-red-300 bg-red-950/60 border border-red-900 rounded-md px-3 py-1 cursor-default">${t("chart.allMonths")}</button>`}
      </div>
      <p class="text-[11px] text-slate-500 mb-3 relative z-10">${t("chart.scrapInteractif.sub")}</p>
...
function renderExportPage(state, data) {
  ...
  <div class="space-y-6 max-w-2xl">
    <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      ...
      <p class="text-xs text-slate-500 mb-4">Télécharge le fichier <code class="font-mono bg-slate-100 px-1.5 py-0.5 rounded">.sqlite</code> complet (6 tables).</p>
