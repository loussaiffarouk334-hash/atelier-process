...
function drawLineChartDark(canvas, labels, values, opts = {}) {
  ...
  const padding = { top: 58, right: 56, bottom: 36, left: 44 };
  ...
  function drawFrame(t) {
    // Garde-fou : si ce canvas a été retiré du DOM (re-render complet du dashboard,
    // changement de page, drill mois→jour→heure→minute...), on arrête la boucle
    // d'animation au lieu de la laisser tourner indéfiniment en arrière-plan.
    // C'était la cause du bug de "curseur" : plusieurs boucles d'animation
    // orphelines dessinaient en même temps sur le même canvas avec des
    // géométries différentes (tailles/échelles obsolètes), ce qui faisait
    // apparaître le tooltip/le curseur décalé par rapport à la courbe.
  ...
  // ⚠️ BUG CORRIGÉ : on stockait auparavant { rafId: null, ... } sur le canvas,
  // une copie figée qui ne reflétait jamais le vrai id de frame courant
  // (state.rafId est mis à jour à CHAQUE frame dans drawFrame). Résultat :
  // au repaint suivant (resize, drill down/up...), cancelAnimationFrame(null)
  // ne faisait rien et l'ancienne boucle continuait de tourner en parallèle
  // de la nouvelle, dessinant un curseur/tooltip à une position obsolète.
  // En pointant canvas._scrapAnim vers le même objet `state`, l'id lu au
  // nettoyage est toujours le dernier id réellement programmé.
  state.onMove = onMove;
  state.onLeave = onLeave;
  canvas._scrapAnim = state;
